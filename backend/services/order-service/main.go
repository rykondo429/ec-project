...
import (
	...
	"ec-sample/services/order-service/infrastructure/mysql"
	"ec-sample/services/order-service/interface/handler"
...
func main() {
	...
	orderRepo = mysql.NewMySQLRepository(db)
	...
	api := e.Group("/api/v1")
	generated.RegisterHandlers(api, orderHandler)
	...
	logger.Info(fmt.Sprintf("Order Service starting on port %s", port))
