...
import (
	...
	"ec-sample/services/point-service/infrastructure/mysql"
	"ec-sample/services/point-service/interface/handler"
...
func main() {
	...
	// マイグレーション実行
	if err := db.AutoMigrate(&domain.PointBalance{}, &domain.PointTransaction{}); err != nil {
		logger.Fatal("Migration failed", zap.Error(err))
	}

	// リポジトリ初期化
	var pointRepo domain.PointRepository
	pointRepo = mysql.NewMySQLRepository(db)

	// ユースケース初期化
	pointUC := usecase.NewPointUseCase(pointRepo)

	// ハンドラー初期化
	pointHandler := handler.NewPointHandler(pointUC)
	...
	api := e.Group("/api/v1")
	generated.RegisterHandlers(api, pointHandler)
	...
	logger.Info(fmt.Sprintf("Point Service starting on port %s", port))
